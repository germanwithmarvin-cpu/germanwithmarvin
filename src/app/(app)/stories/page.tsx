              ...
              <section key={lv} className="space-y-3">
                ...
                <div className="space-y-3">
                  {items.map((s) => {
                    ...
                    const inner = (
                      <div className="card p-5 h-full flex items-center justify-between gap-4 transition hover:border-gold/50">
                        <div className="min-w-0">
                          <div className="text-lg font-semibold">{s.title}</div>
                          {s.intro && <div className="text-sm text-cream-dim mt-1">{s.intro}</div>}
                        </div>
                        <div className="text-2xl shrink-0">{hasText ? "📖" : "📕"}</div>
                      </div>
                    );

                    const main = hasText ? (
                      <Link href={`/stories/${s.id}`} className="flex-1">{inner}</Link>
                    ) : s.fileUrl ? (
