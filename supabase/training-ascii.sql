...
-- Die Aufgabendaten liegen als JSON, damit neue Aufgabentypen spaeter KEINE
-- Datenbank-Aenderung brauchen.
-- ============================================================================
...
create table if not exists public.tr_units (
  ...
  theory text not null default '',          -- Erklaerung (Markdown-aehnlich, einfache Absaetze)
  lesson_id text,                           -- optionale Verknuepfung zur Video-Lektion
...
with u as (
  ...
  values (
    ...
    1,
    E'In a German main clause the conjugated verb always stands in **position 2**. Not second word - second *position*. A position can be one word ("Ich") or a whole phrase ("Am n\u00e4chsten Montag").
...
from u, (values
  ...
  ('order',
   ...
   E'{"tokens":["bin","weil","m\u00fcde","ich"]}',
   E'{"order":["weil","ich","m\u00fcde","bin"]}',
  ...
  ('error',
   ...
   '{}',
   E'{"answers":["Ich bleibe zu Hause, weil ich m\u00fcde bin.","Ich bleibe zu Hause weil ich m\u00fcde bin."]}',
   E'"weil" starts a subordinate clause, so the verb "bin" has to go to the very end: ..., weil ich m\u00fcde *bin*.',
